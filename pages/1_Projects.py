...
def main():
  ...
  with st.container(border=True):
    jobcol = st.columns([2, 5, 2]) 
    a = {"name": "Virtual Doctor's Chatbot", "tech": None, "date":"10/2021", "platform": "IBM Cloud, WatsonX Assistant", "des":"A Virtual Dermatology Doctor's help desk Chatbot built using the concepts of Natural Language Understanding, to consult with patients for making basic diagnosis, and book appointments with the doctor. Made as a semester group project with fellow assignment group members.", "link":"https://web-chat.global.assistant.watson.cloud.ibm.com/preview.html?region=au-syd&integrationID=2b09bf42-de96-41bc-a833-b44e980e9f71&serviceInstanceID=0edb321e-af42-465d-9e5a-821390fade5f", "image": None, "video":"https://youtu.be/w8QHDz1BujM"}
    jobcol[0].write(a["name"])
    jobcol[1].write(f":gray[A Virtual Dermatology Doctor's help desk Chatbot. Made as a semester project :green[Date: {a['date']}]]")
  ...
  with st.container(border=True):
    jobcol = st.columns([2, 5, 2]) 
    a = {"name": "E-Cardo Pizza Chatbot", "tech": None, "date":"10/2021", "platform": "IBM Cloud, WatsonX Assistant", "des":"A virtual assistant Chatbot that helps the customers of a fictional pizza resturant, Ricardo's Pizza, make pizza orders throght the online chatbot infertface, for pizza delivery. Made during semester practical lab.", "link":"https://web-chat.global.assistant.watson.appdomain.cloud/preview.html?region=au-syd&integrationID=a6ad1239-f38a-4587-bf15-aad3eacfc269&serviceInstanceID=efe89b5c-c3f4-4df8-a6f2-c8916b85cc80", "image": "./assets/E-Cardo.jpeg", "video":None}
    jobcol[0].write(a["name"])
    jobcol[1].write(f":gray[E-Cardo, a virtual assistant Chatbot for Ricardo's Pizza Inc. Made for semester practical lab :green[Date: {a['date']}]]")
    if jobcol[2].button(":red[View Details]", use_container_width=True, key="Edardo"):
      project(a["name"], a["date"], a["tech"], a["platform"], a["des"], a["link"], a["image"], a["video"])
  ...
  st.markdown("To see the full list of my projects, visit my [GitHub account](https://github.com/alisterbaroi) ➚", unsafe_allow_html=True)
  with st.container(border=True):
    st.image("./assets/github_banner.png")

  with st.container(border=False):
    jobcol = st.columns([2, 5, 2]) 
    with open("./pages/github.html", "r") as f:
      components.html(f.read(), height=280)
      jobcol[2].button(":red[View Details]", use_container_width=True, key="as")  
